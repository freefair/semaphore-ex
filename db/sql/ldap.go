...
func (d *SqlDb) SaveLDAPProvider(provider db.LDAPProvider) error {
	...
	case err == nil:
		_, err = d.exec(
			...
			provider.NameAttribute, provider.EmailAttribute, provider.ReadinessStatus,
			provider.ReadinessCode, provider.ReadinessCheckedAt, provider.RecoveryAdminUserID,
