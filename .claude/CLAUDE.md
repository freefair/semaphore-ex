...
The `docs` submodule contains directly readable Markdown documentation.
Reading it requires no build, web UI, Node dependencies or hosted configurator.
Keep headings free of visible anchor syntax, and use relative `.md` links.
Start at `docs/README.md`; maintain `docs/docs/CONTENTS.md` when adding pages.
Run `node docs/scripts/check-docs.mjs` for local links, anchors and Markdown syntax.

### Generated references

`docs/docs/reference/configuration.md` and `docs/docs/reference/cli/commands.md`
are committed Markdown generated from `tools/docsref` and `tools/clidocs`.
The API operation/model references are generated by `tools/apidocs`.
Readers never run these tools. Maintainers run `task docs:gen` after changing
configuration or CLI contracts and commit the output with the code.
`task docs:check` checks Markdown and generated-reference freshness.

The configuration reference covers every named-map member as well as scalar
settings. Source comments and reviewed `tools/docsref/descriptions.json` entries
supply explanations; the latter may correct stale comments after checking runtime
consumers. Additional schemas document logger members, the encryption keys file
and native/container-only variables in Markdown.
