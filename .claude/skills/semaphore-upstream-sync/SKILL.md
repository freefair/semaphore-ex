...
- **Migration ownership:** Keep upstream IDs in `db.GetMigrations`, `db/sql/migrations`, and the `migrations` table. EX uses `db.GetEXMigrations`, `db/sql/migrations_ex`, and `ex_migrations` with anchored `X.Y.Z-exA.B.C` identities. Interleave each EX step after its exact upstream anchor and before the next upstream version; validate every anchor and undo in reverse dependency order. Preserve this separation when merging; do not renumber incoming upstream migrations into an EX tail. The format-2 ledger tracks both namespaces and rejects cross-namespace files. Existing mixed-history database adoption is outside the current fresh-development scope.
