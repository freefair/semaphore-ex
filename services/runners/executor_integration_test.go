...
`), 0600))
		...
		provider, err := newExecutorProvider(&util.ExecutorConfig{
			...
			K8s: util.RunnerK8sConfig{
				KubeconfigPath: kubeconfigPath, Context: "test", ClusterAlias: "test",
				ServiceAccount: "semaphore-task",
				Image:          "registry.example.test/task@sha256:" + strings.Repeat("a", 64),
				HelperImage:    "registry.example.test/helper@sha256:" + strings.Repeat("b", 64),
			},
		}, nil)
		if err != nil {
			// In OSS build, stub correctly reports that the executor requires the proprietary build.
