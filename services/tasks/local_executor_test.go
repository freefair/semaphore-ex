...
func TestGetPlaybookArgsDoesNotPassRoutingWrapperAsSSHArgument(t *testing.T) {
	setupExecutorConfig(t)
	executor := LocalExecutor{Template: db.Template{Playbook: "site.yml"}, Inventory: db.Inventory{Type: db.InventoryFile, Inventory: "hosts.ini"}, Repository: db.Repository{GitURL: t.TempDir()}, taskSSHRoutingCommand: "/tmp/task-routing/ssh"}
	args, _, err := executor.getPlaybookArgs("", nil)
	require.NoError(t, err)
	assert.NotContains(t, args, "--ssh-common-args")
}
