...
func (s *ClusterNodeStore) UpsertClusterNode(node pro_interfaces.ClusterNodeRegistration) error {
	if s.connection == nil {
	...
	updated, err := s.updateClusterNode(node, string(capabilities))
	if err != nil || updated {
		return err
	}
	...
	updated, updateErr := s.updateClusterNode(node, string(capabilities))
...
func (s *ClusterNodeStore) updateClusterNode(node pro_interfaces.ClusterNodeRegistration, capabilities string) (bool, error) {
...
func (s *ClusterNodeStore) ListClusterNodes() ([]pro_interfaces.ClusterNodeRegistration, error) {
	if s.connection == nil {
		return nil, fmt.Errorf("cluster node database connection is required")
	}
	var records []clusterNodeRecord
	if _, err := s.connection.SelectAll(&records, "select * from cluster__node order by last_seen_at desc, boot_id desc"); err != nil {
		return nil, err
	}
...
func (s *ClusterNodeStore) SetClusterNodeDraining(bootID string, draining bool) error {
	result, err := s.connection.Exec("update cluster__node set draining=? where boot_id=?", draining, bootID)
	if err != nil {
		return err
	}
...
func (s *ClusterNodeStore) DeleteClusterNodesLastSeenBefore(before time.Time) (int, error) {
	result, err := s.connection.Exec("delete from cluster__node where last_seen_at<?", before)
	if err != nil {
		return 0, err
	}
